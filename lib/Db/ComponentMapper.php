...
final class ComponentMapper extends QBMapper{
 ...
 public function findById(int $ws,int $id,bool $includeDeleted=false):Component{$q=$this->db->getQueryBuilder();$q->select('*')->from('maint_components')->where($q->expr()->eq('workspace_id',$q->createNamedParameter($ws,IQueryBuilder::PARAM_INT)))->andWhere($q->expr()->eq('id',$q->createNamedParameter($id,IQueryBuilder::PARAM_INT)));if(!$includeDeleted)$q->andWhere($q->expr()->isNull('deleted_at'));return $this->findEntity($q);}
}
