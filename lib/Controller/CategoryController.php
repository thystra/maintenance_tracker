...
final class CategoryController extends OCSController {
	...
	public function index(?string $workspace=null):DataResponse{try{return $this->workspaceService->runWithAccess($this->currentUser->uid(),$workspace,'viewer',fn($c)=>new DataResponse(['workspace'=>$c->workspace()->getUuid(),'items'=>$this->service->list($c)]));}catch(AccessDeniedException $e){throw new OCSForbiddenException($e->getMessage(),$e);}}
	#[NoAdminRequired] #[ApiRoute(verb:'POST',url:'/api/v1/categories')]
	public function create(array $category,?string $workspace=null):DataResponse{try{return $this->workspaceService->runWithAccess($this->currentUser->uid(),$workspace,'editor',fn($c)=>new DataResponse($this->service->create($c,$category)->toApi(),Http::STATUS_CREATED));}catch(AccessDeniedException $e){throw new OCSForbiddenException($e->getMessage(),$e);}catch(ValidationException $e){throw new OCSBadRequestException($e->getMessage(),$e);}}
}
