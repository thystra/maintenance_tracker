...
fixture_paths=(
	.gitattributes .nextcloudignore appinfo/info.xml composer.json package.json
...
PY
expect_rejected 'inconsistent application versions' 'App version must match appinfo/info.xml, Application::APP_VERSION, and package.json.' "$tmp/version.out"
...
replacement="\t\t\tself::WORKSPACE_MEMBERS_READ,\n\t\t\tself::WORKSPACE_MEMBERS_MANAGE,\n\t\t\tself::AUDIT_READ,\n\t\t],\n\t\t'contributor'"
if needle not in s: raise SystemExit('manager fixture marker missing')
