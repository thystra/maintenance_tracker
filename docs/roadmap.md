...
10. [ ] v0.1.10 canonical parts plus portable fitment packs and compatibility matrix import/export: JSON plus bounded CSV/ZIP validation/import/export, standardized service-position persistence, equipment matching/manual mapping, source export, and privacy-minimized community export are implemented in the current runtime checkpoint; profile-v2 part materialization, activity part usage, vendor management, safe store-link management, and the central cost ledger remain pending;
