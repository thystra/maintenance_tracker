...
### Fitment runtime authorization and privacy

The v0.1.10 fitment runtime keeps validation/read access separate from writes: all workspace roles may use `fitment.read`, while only Owner/Manager receive `fitment.import` and `fitment.map`. Import does not automatically attach a generalized target to a local asset. `conflict` and `insufficient` matches require explicit confirmation. Source/store URLs are metadata and are never fetched server-side. Community export is rebuilt from reviewed generalized descriptors and canonical fitment facts and excludes local unit UUIDs/nicknames, VIN/serial identity, notes, costs, receipts, and maintenance history. Offers are omitted from community export until a reviewed selection policy exists.

CSV/ZIP is a projection into that same validator, not a second trust boundary. Import reads only the exact root-level allowlist into bounded memory and never extracts archive paths. It rejects traversal/path separators, symlinks, duplicate/unknown/missing names, comments, encryption, unsupported compression, more than 8 MiB compressed/32 MiB expanded data, incorrect headers, invalid UTF-8/BOMs, and more than 250,000 aggregate data rows. `equipment_identifiers.csv` preserves reusable model/configuration identifiers so round trips do not silently discard canonical facts. Formula-sensitive cells are escaped only with the manifest-declared reversible `leading-apostrophe-v1` scheme; canonical JSON never contains spreadsheet escaping.
